function [B,N,fn] = build_bndq(BC,Grid,I,zN,Nbs)
% author: Marc Hesse
% date: 06/09/2015
% Description:
...
B=I(BC.dof_dir-Nbs,:);
N=I; N(:,BC.dof_dir-Nbs)=[];

%fn = spalloc(zN,1,0); % empty for now because we don't yet have Neuman BC's

%% Neumann boundary conditions
if isempty(BC.dof_neu)
            fn = spalloc(zN,1,0);     % allocate sparse zero vector        
else
     fn = spalloc(zN,1,2);     % allocate sparse zero vector
    fn(BC.dof_neu,1)=BC.qb.*Grid.A(BC.dof_f_neu)./ Grid.V(BC.dof_neu);
