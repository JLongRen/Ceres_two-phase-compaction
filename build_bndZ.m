function [B,N,fn] = build_bndZ(BC,Grid,I,n1,n2)
% author: Marc Hesse
% date: 06/09/2015
% Description:
...
B=I(BC.dof_dir-n1+1,:);
N=I; N(:,BC.dof_dir-n1+1)=[];

%fn = spalloc(zN,1,0); % empty for now because we don't yet have Neuman BC's

%% Neumann boundary conditions
if isempty(BC.dof_neu)
            fn = spalloc(n2-n1+1,1,0);     % allocate sparse zero vector        
else
     fn = spalloc(n2-n1+1,1,2);     % allocate sparse zero vector
    fn(BC.dof_neu-n1+1,1)=BC.qb.*Grid.A(BC.dof_f_neu)./ Grid.V(BC.dof_neu);
